...
class MainActivity : AppCompatActivity() {
    ...
    private fun attachServiceCallbacks() {
        playerService?.apply {
            ...
            onSnapStateChanged = { connected ->
                runOnUiThread {
                    binding.indicatorSnap.setState(
                        if (connected) {
                            StatusIndicatorView.State.CONNECTED
                        } else {
                            StatusIndicatorView.State.OFFLINE
                        }
                    )

                    if (!connected && currentOverlay == OverlayState.RADIO) {
                        hideOverlay()
                    }
                }
            }
            ...
            // Aktivitás LED pulzálás.
            onSnapActivity = {
                runOnUiThread {
                    binding.indicatorSnap.pulse()
                }
            }

            onNetActivity = {
                runOnUiThread {
                    binding.indicatorNet.pulse()
                }
            }
    ...
    private fun handleBell(event: BellEvent) {
        if (!isThisDeviceTargeted(event.unmutedDeviceIds)) return
    ...
    private fun handleTts(event: TtsEvent) {
        ...
        val displayText = when {
            ...
            else -> "Hangos közlemény"
        }

        showMessageOverlay(displayText, event.durationMs)
    }

    private fun handleRadio(event: RadioEvent) {
        if (currentOverlay == OverlayState.BELL || currentOverlay == OverlayState.MESSAGE) return
        if (!event.snapActive) return
        if (!isThisDeviceTargeted(event.unmutedDeviceIds)) return
    ...
    private fun isThisDeviceTargeted(unmutedDeviceIds: List<String>): Boolean {
        if (unmutedDeviceIds.isEmpty()) return true

        val thisDeviceId = PrefsUtil.getDeviceId(this)
        if (thisDeviceId.isEmpty()) return false

        return unmutedDeviceIds.contains(thisDeviceId)
    }
    ...
    private fun hideAllOverlaysInternal() {
        listOf(
            binding.overlayBell,
            binding.overlayMessage,
            binding.overlayRadio
        ).forEach { v ->
            v.animate()
                .alpha(0f)
                .setDuration(200)
    ...
    private fun setupUi() {
        binding.tvAppName.text = "SchoolLive! client V${BuildConfig.VERSION_NAME}"
    ...
    private fun startClock() {
        ...
        val tick = object : Runnable {
            override fun run() {
                ...
                binding.bellPill.visibility = if (next != null) {
                    View.VISIBLE
                } else {
                    View.GONE
                }
                ...
                if (resp.isSuccessful) {
                    ...
                    if (!name.isNullOrEmpty()) {
                        withContext(Dispatchers.Main) {
                            binding.tvTenantName.text = name
                        }

                        PrefsUtil.setDeviceName(this@MainActivity, name)
                    }

                    // DeviceId perzisztálás.
            ...
            try {
                ...
                if (resp.isSuccessful && resp.body()?.status != "active") {
                    withContext(Dispatchers.Main) {
                        ...
                        startActivity(
                            Intent(this@MainActivity, ProvisioningActivity::class.java).apply {
